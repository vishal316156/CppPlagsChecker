...
import { createNGrams } from "../src/services/ngram.service.js";
import { calculateJaccardSimilarity } from "../src/services/similarity.service.js";
import { createWinnowingFingerprints } from "../src/services/winnowing.service.js";
...
function calculateScores(codeA, codeB) {
  ...
  const ngramScore = shortScore * 0.4 + longScore * 0.6;
  ...
  const winnowingScore =
    calculateJaccardSimilarity(fingerprintsA, fingerprintsB);
...
describe("C++ similarity benchmark dataset", () => {
  for (const pair of benchmarkPairs) {
    test(pair.name, () => {
      ...
      expect(scores.ngramScore).toBeGreaterThanOrEqual(0);
      expect(scores.ngramScore).toBeLessThanOrEqual(1);

      expect(scores.winnowingScore).toBeGreaterThanOrEqual(0);
      expect(scores.winnowingScore).toBeLessThanOrEqual(1);
    });
  }
});

describe("benchmark rule evaluation", () => {
  test("evaluates winnowing threshold", () => {
    const results = benchmarkPairs.map((pair) => {
      const scores = calculateScores(pair.codeA, pair.codeB);
    ...
    const metrics = evaluatePredictions(results);

    console.log("Evaluation metrics:", metrics);

    expect(metrics.truePositive).toBe(6);
    expect(metrics.falseNegative).toBe(0);
    expect(metrics.falsePositive).toBe(0);
    expect(metrics.trueNegative).toBe(4);
  });
});